...
function onlineUserList(messages) {
    ...
    for (var user in messages) {
        tableOnlineUsers += '<a href="/profile/ShowProfileServlet?user=' + messages[user].username + '" onclick=\"window.open(\'/profile/ShowProfileServlet?user=' + messages[user].username + '\',\'Help\',\'width=520,height=305,scrollbars=no,toolbar=no,location=no\'); return false">' + messages[user].username + '</a>';
        ;
    }

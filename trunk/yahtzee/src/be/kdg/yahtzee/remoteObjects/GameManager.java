...
import be.kdg.yahtzee.dao.HighscoreDaoImpl;
...
public class GameManager {
    ...
    private HighscoreDaoImpl highscoreDao;
    ...
    public GameManager() {
        ...
        GlobalChat = new Chat();
        this.highscoreDao = new HighscoreDaoImpl();
    ...
    public static GameManager getInstance() throws RemoteException {
        if (gameManager == null) {
    ...
    public void saveHighscore(User user, int score, Date timestamp) {
        ...
        highscores.add(highscore);
        logger.info("Highscore " + score + " van user " + user.getUsername() + " saved");
    }
    ...
    public boolean joinGame(String gameName, User user) {
        Game game = getGame(gameName);
        if (checkUserAlreadyInAgame(user)) {
            logger.debug("Game : " + gameName + " kon niet worden gejoined reden: de user doet al mee aan een ander spel");
            return false;
        }
        if (checkGameFullBusy(game)) {
            logger.debug("Game : " + gameName + " kon niet worden gejoined reden: de game is full of busy");
            return false;
        }
        if (checkChangeStatusToFull(game)) {
            game.setState(EnumState.VOL);
            logger.debug("Game status Vol");
        }
    ...
    public String startGame(String gamename) {
        Game game = getGame(gamename);
        game.startGame();
        logger.debug("Game " + gamename + " Started");

...
public class YahtzeeSoapBindingImpl implements be.kdg.yahtzee.remoteObjects.YahtzeeController {
    ...
    private java.lang.Object[] convertMap(Map<String, Role> scoreMap) {
        ...
        for (Iterator it = scoreMap.entrySet().iterator(); it.hasNext();) {
            ...
            be.kdg.yahtzee.remoteObjects.users.Role value = convertRoleObject((Role) entry.getValue());
    ...
    private java.lang.Object[] convertListUsers(List<User> userlist) {
        ...
        for (User user : userlist) {
            be.kdg.yahtzee.remoteObjects.users.User userRem = convertUserObject(user);
    ...
    private java.lang.Object[] convertListScorePosibilities(List<ScoreAspect> scoreposlist) {
        ...
        for (ScoreAspect scoreAspect : scoreposlist) {
            be.kdg.yahtzee.remoteObjects.game.ScoreAspect scoreaspectRem = convertScoreAspectObject(scoreAspect);
    ...
    private java.lang.Object[] convertListDies(List<Die> dielist) {
        List returnlist = new ArrayList();
        for (Die die : dielist) {
            be.kdg.yahtzee.remoteObjects.game.Die dieRem = convertDieObject(die);
    ...
    private java.lang.Object[] convertListHighscores(List<Highscore> highscorelist) {
        ...
        for (Highscore highscore : highscorelist) {
            be.kdg.yahtzee.remoteObjects.game.Highscore highScoreRem = convertHighsScoreObject(highscore);
    ...
    private java.lang.Object[] convertSetGames(Set<Game> gameSet) {
        ...
        for (Game game : gameSet) {
            be.kdg.yahtzee.remoteObjects.game.Game gameRem = convertGameObject(game);
            returnlist.add(gameRem);
        }
        return returnlist.toArray();
    }

    private be.kdg.yahtzee.remoteObjects.game.Die[] convertListDie(List<Die> dielist) {
        be.kdg.yahtzee.remoteObjects.game.Die[] returnlist = new be.kdg.yahtzee.remoteObjects.game.Die[0];
        int i = 0;
        for (Die die : dielist) {
            be.kdg.yahtzee.remoteObjects.game.Die dieRem = convertDieObject(die);
            returnlist[i] = dieRem;
        ...
        //be.kdg.yahtzee.remoteObjects.game.Die[] dieqsdf = (be.kdg.yahtzee.remoteObjects.game.Die[]) returnlist.toArray();
    ...
    /*
       ...
       for(ScoreAspect scoreAspect: scorelist){
           be.kdg.yahtzee.remoteObjects.game dieRem = convertDieObject(die) ;
           returnlist.add(dieRem);
       }
       return returnlist.toArray();
   } */
    ...
    private be.kdg.yahtzee.model.users.Address convertAddressRemObject(be.kdg.yahtzee.remoteObjects.users.Address remAddress) {
    ...
    private be.kdg.yahtzee.model.users.Person convertPersonRemObject(be.kdg.yahtzee.remoteObjects.users.Person remPerson) {
    ...
    private be.kdg.yahtzee.model.users.Role convertRoleRemObject(be.kdg.yahtzee.remoteObjects.users.Role remRole) {
        return userManager.getRole(remRole.getName());
        //return new be.kdg.yahtzee.model.users.Role(remRole.getName());
    }

    private be.kdg.yahtzee.model.users.User convertUserRemObject(be.kdg.yahtzee.remoteObjects.users.User remUser) {
    ...
    private be.kdg.yahtzee.model.game.Game convertGameRemObject(be.kdg.yahtzee.remoteObjects.game.Game remGame) {
    ...
    private be.kdg.yahtzee.model.game.Die convertDieRemObject(be.kdg.yahtzee.remoteObjects.game.Die remDie) {
        return new be.kdg.yahtzee.model.game.Die(remDie.getDieId());
    }

    private be.kdg.yahtzee.model.game.Score convertScoreRemObject(be.kdg.yahtzee.remoteObjects.game.Score remScore) {
        return new be.kdg.yahtzee.model.game.Score();
    }

    // ------chat objecten

    private be.kdg.yahtzee.model.chat.Chat convertChatRemObject(be.kdg.yahtzee.remoteObjects.chat.Chat remChat) {
    ...
    private be.kdg.yahtzee.remoteObjects.users.Address convertAddressObject(be.kdg.yahtzee.model.users.Address address) {
        return new be.kdg.yahtzee.remoteObjects.users.Address(address.getCity(), address.getCountry(), address.getNumber(), address.getStreet(), address.getZip());
    }

    private be.kdg.yahtzee.remoteObjects.users.Person convertPersonObject(be.kdg.yahtzee.model.users.Person person) {
        return new be.kdg.yahtzee.remoteObjects.users.Person(convertAddressObject(person.getAddress()), person.getEmail(), person.getFirstName(), person.getSurname(), person.getTelephone());
    }

    private be.kdg.yahtzee.remoteObjects.game.Highscore convertHighsScoreObject(be.kdg.yahtzee.model.game.Highscore highscore) {
        return new be.kdg.yahtzee.remoteObjects.game.Highscore(highscore.getHighscoreId(), highscore.getScore(), convertDate(highscore.getTimestamp()), convertUserObject(highscore.getUser()));
    }

    private be.kdg.yahtzee.remoteObjects.users.Role convertRoleObject(be.kdg.yahtzee.model.users.Role role) {
        return new be.kdg.yahtzee.remoteObjects.users.Role(role.getName(), role.getRoleId());
    }

    private be.kdg.yahtzee.remoteObjects.users.User convertUserObject(be.kdg.yahtzee.model.users.User user) {
        return new be.kdg.yahtzee.remoteObjects.users.User(convertAddressObject(user.getAddress()),
                user.isBlocked(),
                user.getEmail(),
                user.getFirstname(),
                user.isOnline(),
                user.getPassword(),
                convertPersonObject(user.getPerson()),
                convertRoleObject(user.getRole()),
                user.getSurname(),
                user.getTelephone(),
                user.getUserId(),
                user.getUsername());
    ...
    private be.kdg.yahtzee.remoteObjects.game.Game convertGameObject(be.kdg.yahtzee.model.game.Game game) {
        ...
        return new be.kdg.yahtzee.remoteObjects.game.Game(
    ...
    private be.kdg.yahtzee.remoteObjects.game.Die convertDieObject(be.kdg.yahtzee.model.game.Die die) {
        return new be.kdg.yahtzee.remoteObjects.game.Die(die.isDieFixed(), die.isDieFixed(), die.getDieId(), die.getValue());
    }

    private be.kdg.yahtzee.remoteObjects.game.ScoreAspect convertScoreAspectObject(ScoreAspect scoreAspect) {
        return new be.kdg.yahtzee.remoteObjects.game.ScoreAspect(scoreAspect.getDescription(), scoreAspect.isFixed(), scoreAspect.getPoints());
    }

    private be.kdg.yahtzee.remoteObjects.game.Score convertScoreObject(be.kdg.yahtzee.model.game.Score score) {
        return new be.kdg.yahtzee.remoteObjects.game.Score(
    ...
    private be.kdg.yahtzee.remoteObjects.chat.Chat convertChatObject(be.kdg.yahtzee.model.chat.Chat chat) {
        return new be.kdg.yahtzee.remoteObjects.chat.Chat();
    }
